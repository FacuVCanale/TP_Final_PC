...
directions ={}
directions['Ivan'] = {'speed': 30, 'direction': 45}
directions['Marcos'] = {'speed': 10, 'direction': 50}

cliente.add_team("EQUIPO2",['Ivan','Marcos'])
...
def mandar_data():
    while not cliente.is_over():
        ...
        cliente.next_iteration("EQUIPO2", directions)  # VER POR QUE FACU FALLECE Y NO SE PRINTEA EN GET.DATA(), ES DECIR, NO APARECE EN EL SV.
        ...
        directions['Ivan']['direction'] += random.choice([i for i in range(300)])
        directions['Marcos']['direction'] += random.choice([i for i in range(200)])
